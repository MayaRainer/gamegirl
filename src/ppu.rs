...
fn load_pixel(sys: &mut System, lcd_control: u8, tile_map_bit: u8, y: u8, x: u8) -> u8 {
	...
	let tile_address = if (lcd_control & 1 << 4) != 0 {
		0x8000 + u16::from(index) * 16
	} else {
		0x9000u16.wrapping_add_signed(i16::from(index.cast_signed()) * 16)
	};
	load_tile_line(sys, tile_address, y)[(x % 8) as usize]
}
	...
	pub(crate) fn step(&mut self, sys: &mut System) {
		...
		match self.mode {
			...
			mode::DRAWING => {
				...
				let bg: [u8; SCREEN_WIDTH] = std::array::from_fn(|x| {
					let x = x as u8;
					if lcd_control & (1 << 5) != 0 && x + 7 >= window_x && self.line >= window_y {
						if x == window_x - 7 {
							self.window_line = self.window_line.wrapping_add(1);
						}
						load_pixel(sys, lcd_control, 6, self.window_line, x + 7 - window_x)
					} else if lcd_control & 1 != 0 {
						load_pixel(sys, lcd_control, 3, self.line.wrapping_add(scy), x.wrapping_add(scx))
					} else { 0 }
				});
				
				let mut rendered_objects = [0u8; SCREEN_WIDTH];
				if lcd_control & (1 << 1) != 0 {
					...
					for object in objects_on_line.into_iter().take(10) {
						...
						for i in 8usize.saturating_sub(object_x)..(SCREEN_WIDTH + 8).saturating_sub(object_x).min(8) {
							...
							if colour_id != 0 && *target == 0 && (object[3] & (1 << 7) == 0 || bg[x] == 0) {
								// Adding 1 as 0 means "not occupied by an object" - we handle that case below
								*target = 1 + colour_from_palette(if object[3] & (1 << 4) != 0 { obj_palette_2 } else { obj_palette_1 }, colour_id);
							}
						}
					}
				}
				...
				for x in 0..SCREEN_WIDTH {
					let pixel = if rendered_objects[x] != 0 {
						rendered_objects[x] - 1
					} else {
